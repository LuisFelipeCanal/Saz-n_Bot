...
def get_system_prompt(menu, distritos):
    ...
    **IMPORTANTE: Validación de cantidad solicitada**
    	Extrae la cantidad del pedido, que puede estar en formato de string (por ejemplo, "10").Convierte esa cantidad a un entero.Verifica si la cantidad está en el rango de 1 a 100. Si la cantidad es válida (es decir, está entre 1 y 100), responde: "Has pedido {cantidad} unidades.".Si la cantidad no es válida, responde: "La cantidad debe estar entre 1 y 100. Por favor, intenta de nuevo.".
    ...
    *Después de que el cliente agrega bebidas o postres, pregúntale si desea agregar algo más.* Si el cliente desea agregar más platos, bebidas o postres, permite que lo haga. Si no desea agregar más, continúa con el proceso.
    ...
    Si el cliente confirma el pedido, solicita al cliente el método de pago (tarjeta, efectivo, Yape u otra opción disponible). Asegurate que el cliente haya ingresado un método de pago antes de continuar.
